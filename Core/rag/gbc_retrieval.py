    ...
    def text_reranker(
        ...
        for node in subtree_nodes:
            ...
            if node_type == NodeType.TABLE:
                # Use the pre-converted content from tree stage
                text = node.meta_info.content
                if not text and node.meta_info.table_body:
                    # Fallback for old tree.json files without content field
                    table_data = {
                        "caption": node.meta_info.caption or "",
                        "table_body": node.meta_info.table_body,
                        "footnote": node.meta_info.footnote or "",
                    }
                    text = table2text(table_data)
            else:
                text = node.meta_info.content
